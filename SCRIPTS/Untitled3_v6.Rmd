...
hist_data <- paper3_data %>% 
  dplyr::select(Platform , SI) %>% 
  group_by(Platform) %>% 
  summarise(mean = mean(SI) , sd = sd(SI) , median = median(SI))

hist_data_gs <- hist_data %>% 
  filter(Platform == "GreenSeeker NDVI")

lower_limit <- hist_data_gs[2] - (4*hist_data_gs[3]) #upper limit for outlier removal. Observations that are 4 sd's away from mean. Observations less than 0.1997493 in GreenSeeker. 
...
round(tapply(paper3_gsdata$PI_N_Uptake , paper3_gsdata$site_year , min) , digits = 1)
round(tapply(paper3_gsdata$GrainYield_Mgha , paper3_gsdata$site_year , max) , digits = 1)

round(tapply(paper3_gsdata$GrainYield_Mgha , paper3_gsdata$site_year , min) , digits = 1)
round(tapply(paper3_gsdata$GrainYield_Mgha , paper3_gsdata$site_year , max) , digits = 1)

round(min(paper3_uas_ndvi_data$SI) , digits = 2)
round(max(paper3_uas_ndvi_data$SI) , digits = 2)

round(min(paper3_gsdata$SI) , digits = 2)
round(max(paper3_gsdata$SI) , digits = 2)

round(min(paper3_uas_ndre_data$SI) , digits = 2)
round(max(paper3_uas_ndre_data$SI) , digits = 2)

round((length(which(paper3_uas_ndvi_data$SI >= .85)) / length(paper3_uas_ndvi_data$SI)) * 100 , digits = 0)
round((length(which(paper3_uas_ndre_data$SI >= .85)) / length(paper3_uas_ndre_data$SI)) * 100 , digits = 0)
round((length(which(paper3_gsdata$SI >= .85)) / length(paper3_gsdata$SI)) * 100 , digits = 0)

test <- paper3_uas_ndre_data %>% 
  filter(SI >= .90)

test <- round((sd(test$relative_grain_yield)*100) , digits = 1)

test

test <- paper3_uas_ndvi_data %>% 
  filter(SI >= .90)
...
round((length(which(paper3_uas_ndvi_data$SI >= .9)) / length(paper3_uas_ndvi_data$SI)) * 100 )
round((length(which(paper3_uas_ndre_data$SI >= .9)) / length(paper3_uas_ndre_data$SI)) * 100 )
round((length(which(paper3_gsdata$SI >= .9)) / length(paper3_gsdata$SI)) * 100 )
...
factor_list1 <- list( site = suppl_data1$site_year , rate = suppl_data1$N_level_kgha)
factor_list2 <- list( site = suppl_data1$site_year)

round(tapply(suppl_data1$PI_N_Uptake, factor_list1 , min) , digits = 0)
round(tapply(suppl_data1$PI_N_Uptake, factor_list2 , max) , digits = 0)

tapply(suppl_data1$GrainYield_Mgha_r, factor_list1 , min)
tapply(suppl_data1$GrainYield_Mgha_r, factor_list2 , max)
...
nic17_plot_legend <- ggplot(data = suppl_data1_nic17 , aes( x = N_level_kgha , y = Unit , color = Legend , linetype = Legend)) +
  geom_point(data = suppl_data1_nic17 , aes ( x = N_level_kgha , y = Unit , color = Legend) , size = 7) +
  ...
  guides(color = guide_legend(keywidth = 9.5 , keyheight = 1.5 , unit = "cm" , title.hjust = 0.5)) +
  scale_color_manual("Legend" , labels = c("Grain Yield" , "PI N Uptake") , values = c( "#00BFC4" , "#F8766D" )) +
  scale_linetype_manual("Legend" , labels = c("Grain Yield" , "PI N Uptake") , values = c( "dashed" , "solid" ))
...
nic17_a <- as.numeric(as.character(coef(nic17_yield_lm)[3]))
nic17_b <- as.numeric(as.character(coef(nic17_yield_lm)[2]))
nic17_c <- as.numeric(as.character(coef(nic17_yield_lm)[1]))
nic17sym_x <- round((-nic17_b) / (2*nic17_a) , digits = 0)
nic17sym_y <- round(nic17_a*(nic17sym_x^2) + nic17_b*nic17sym_x + nic17_c , digits = 1)
...
nic17_plot <- ggplot(data = suppl_data1_nic17 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_nic17 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_nic17 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = nic17_df , aes( x = suppl_data1_nic17.N_level_kgha , y = nic17_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = nic17_df_2 , aes( x = suppl_data1_nic17.N_level_kgha , y = nic17_fitted_2) , color ="#00BFC4" , size = 1 , linetype = "dashed") +
...
wil17_a <- as.numeric(as.character(coef(wil17_yield_lm)[3]))
wil17_b <- as.numeric(as.character(coef(wil17_yield_lm)[2]))
wil17_c <- as.numeric(as.character(coef(wil17_yield_lm)[1]))
wil17sym_x <- round((-wil17_b) / (2*wil17_a) , digits = 0)
wil17sym_y <- round(wil17_a*(wil17sym_x^2) + wil17_b*wil17sym_x + wil17_c , digits = 1)
...
wil17_plot <- ggplot(data = suppl_data1_wil17 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_wil17 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_wil17 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = wil17_df , aes( x = suppl_data1_wil17.N_level_kgha , y = wil17_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = wil17_df_2 , aes( x = suppl_data1_wil17.N_level_kgha , y = wil17_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
arb18_a <- as.numeric(as.character(coef(arb18_yield_lm)[3]))
arb18_b <- as.numeric(as.character(coef(arb18_yield_lm)[2]))
arb18_c <- as.numeric(as.character(coef(arb18_yield_lm)[1]))
arb18sym_x <- round((-arb18_b) / (2*arb18_a) , digits = 0)
arb18sym_y <- round(arb18_a*(arb18sym_x^2) + arb18_b*arb18sym_x + arb18_c , digits = 1)
...
arb18_plot <- ggplot(data = suppl_data1_arb18 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_arb18 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_arb18 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = arb18_df , aes( x = suppl_data1_arb18.N_level_kgha , y = arb18_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = arb18_df_2 , aes( x = suppl_data1_arb18.N_level_kgha , y = arb18_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
biggs18_a <- as.numeric(as.character(coef(biggs18_yield_lm)[3]))
biggs18_b <- as.numeric(as.character(coef(biggs18_yield_lm)[2]))
biggs18_c <- as.numeric(as.character(coef(biggs18_yield_lm)[1]))
biggs18sym_x <- round((-biggs18_b) / (2*biggs18_a) , digits = 0)
biggs18sym_y <- round(biggs18_a*(biggs18sym_x^2) + biggs18_b*biggs18sym_x + biggs18_c , digits = 1)
...
biggs18_plot <- ggplot(data = suppl_data1_biggs18 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_biggs18 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_biggs18 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = biggs18_df , aes( x = suppl_data1_biggs18.N_level_kgha , y = biggs18_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = biggs18_df_2 , aes( x = suppl_data1_biggs18.N_level_kgha , y = biggs18_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
mry18_a <- as.numeric(as.character(coef(mry18_yield_lm)[3]))
mry18_b <- as.numeric(as.character(coef(mry18_yield_lm)[2]))
mry18_c <- as.numeric(as.character(coef(mry18_yield_lm)[1]))
mry18sym_x <- round((-mry18_b) / (2*mry18_a) , digits = 0)
mry18sym_y <- round(mry18_a*(mry18sym_x^2) + mry18_b*mry18sym_x + mry18_c , digits = 1)
...
mry18_plot <- ggplot(data = suppl_data1_mry18 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_mry18 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_mry18 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = mry18_df , aes( x = suppl_data1_mry18.N_level_kgha , y = mry18_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = mry18_df_2 , aes( x = suppl_data1_mry18.N_level_kgha , y = mry18_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
  theme(axis.text.y.right = element_text(size = 30),
...
nic18_a <- as.numeric(as.character(coef(nic18_yield_lm)[3]))
nic18_b <- as.numeric(as.character(coef(nic18_yield_lm)[2]))
nic18_c <- as.numeric(as.character(coef(nic18_yield_lm)[1]))
nic18sym_x <- round((-nic18_b) / (2*nic18_a) , digits = 0)
nic18sym_y <- round(nic18_a*(nic18sym_x^2) + nic18_b*nic18sym_x + nic18_c , digits = 1)
...
nic18_plot <- ggplot(data = suppl_data1_nic18 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_nic18 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_nic18 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = nic18_df , aes( x = suppl_data1_nic18.N_level_kgha , y = nic18_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = nic18_df_2 , aes( x = suppl_data1_nic18.N_level_kgha , y = nic18_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
arb19_a <- as.numeric(as.character(coef(arb19_yield_lm)[3]))
arb19_b <- as.numeric(as.character(coef(arb19_yield_lm)[2]))
arb19_c <- as.numeric(as.character(coef(arb19_yield_lm)[1]))
arb19sym_x <- round((-arb19_b) / (2*arb19_a) , digits = 0)
arb19sym_y <- round(arb19_a*(arb19sym_x^2) + arb19_b*arb19sym_x + arb19_c , digits = 1)
...
arb19_plot <- ggplot(data = suppl_data1_arb19 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_arb19 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_arb19 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = arb19_df , aes( x = suppl_data1_arb19.N_level_kgha , y = arb19_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = arb19_df_2 , aes( x = suppl_data1_arb19.N_level_kgha , y = arb19_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
davis19_a <- as.numeric(as.character(coef(davis19_yield_lm)[3]))
davis19_b <- as.numeric(as.character(coef(davis19_yield_lm)[2]))
davis19_c <- as.numeric(as.character(coef(davis19_yield_lm)[1]))
davis19sym_x <- (-davis19_b) / (2*davis19_a)
davis19sym_y <- davis19_a*(davis19sym_x^2) + davis19_b*davis19sym_x + davis19_c
...
davis19_plot <- ggplot(data = suppl_data1_davis19 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_davis19 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_davis19 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = davis19_df , aes( x = suppl_data1_davis19.N_level_kgha , y = davis19_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = davis19_df_2 , aes( x = suppl_data1_davis19.N_level_kgha , y = davis19_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
mry19_a <- as.numeric(as.character(coef(mry19_yield_lm)[3]))
mry19_b <- as.numeric(as.character(coef(mry19_yield_lm)[2]))
mry19_c <- as.numeric(as.character(coef(mry19_yield_lm)[1]))
mry19sym_x <- round((-mry19_b) / (2*mry19_a) , digits = 0)
mry19sym_y <- round(mry19_a*(mry19sym_x^2) + mry19_b*mry19sym_x + mry19_c , digits = 1)
...
mry19_plot <- ggplot(data = suppl_data1_mry19 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_mry19 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_mry19 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = mry19_df , aes( x = suppl_data1_mry19.N_level_kgha , y = mry19_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = mry19_df_2 , aes( x = suppl_data1_mry19.N_level_kgha , y = mry19_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
    scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
res19_a <- as.numeric(as.character(coef(res19_yield_lm)[3]))
res19_b <- as.numeric(as.character(coef(res19_yield_lm)[2]))
res19_c <- as.numeric(as.character(coef(res19_yield_lm)[1]))
res19sym_x <- round((-res19_b) / (2*res19_a) , digits = 0)
res19sym_y <- round(res19_a*(res19sym_x^2) + res19_b*res19sym_x + res19_c , digits = 1)
...
res19_plot <- ggplot(data = suppl_data1_res19 , aes( x = N_level_kgha , y = PI_N_Uptake)) +
  geom_point(data = suppl_data1_res19 , aes ( x = N_level_kgha , y = PI_N_Uptake) , color = "#F8766D" , size = 6) +
  geom_point(data = suppl_data1_res19 , aes ( x = N_level_kgha , y = GrainYield_Mgha*23) , color = "#00BFC4" , size = 6) +
  geom_line(data = res19_df , aes( x = suppl_data1_res19.N_level_kgha , y = res19_fitted) , color = "#F8766D" , size = 1) +
  geom_line(data = res19_df_2 , aes( x = suppl_data1_res19.N_level_kgha , y = res19_fitted_2) , color = "#00BFC4" , size = 1 , linetype = "dashed") +
  scale_y_continuous(breaks = seq(0 , 300 , by = 100) , sec.axis = sec_axis(~./23  , breaks = seq(0 , 16 , by = 3))) +
  coord_cartesian(ylim = c(0 , 300) , xlim = c(0 , 250)) +
  scale_x_continuous(breaks = seq(0 , 280 , by = 100)) +
  theme_classic() +
...
Fig2 <- grid.arrange(arrangeGrob(
                              ...
                              bottom = textGrob("Pre-plant N Rate ( kg N ha"^-1~")" ,
                                                gp = gpar( fontsize = 38)),
                              right = textGrob("Grain Yield ( Mg ha"^-1~")" ,
                                              gp = gpar(fontsize = 40) ,
                                              rot = 270),
                              left = textGrob("PI N Uptake ( kg N ha"^-1~")" ,
                                              gp = gpar( fontsize = 36) ,
                                              rot = 90)
                              ))
...
ggsave("FIGURES/Fig2.tiff" , Fig2, compression = "lzw" , width = 22 , height = 12 , type = "cairo" , dpi = 1000)
...
fit.lmS1    <- lm(relative_grain_yield ~ N_level_kgha , data= paper3_gsdata)

a.iniS1     <- fit.lmS1$coefficients[1]
b.iniS1     <- fit.lmS1$coefficients[2]
clx.iniS1   <- mean(paper3_gsdata$N_level_kgha)

quadplat <- function(x, a, b, clx) {
           ifelse(x  < clx, a + b * x   + (-0.5*b/clx) * x   * x,
                            a + b * clx + (-0.5*b/clx) * clx * clx)}

modelS1 <- nls(relative_grain_yield ~ quadplat(N_level_kgha, a, b, clx),
            data = paper3_gsdata,
            start = list(a   = a.iniS1,
                         b   = b.iniS1,
                         clx = clx.iniS1),
             trace = FALSE,
...
m.iniS1    <- mean(paper3_gsdata$N_level_kgha)

nullS1 <- nls(relative_grain_yield ~ nullfunct(N_level_kgha, m),
           data = paper3_gsdata,
           start = list(m   = m.iniS1),
           trace = FALSE,
           nls.control(maxiter = 1000))
...
BootS1 <- nlsBoot(modelS1)

summary(BootS1)

plotPredy(data  = paper3_gsdata,
          x     = N_level_kgha,
          y     = relative_grain_yield,
          model = modelS1,
          xlab  = "Pre-plant N Rate",
          ylab  = "Relative Grain Yield")

xS1 <- residuals(modelS1)

plotNormalHistogram(xS1)

plot(fitted(modelS1),
     residuals(modelS1))

aS1 <- summary(modelS1)$coefficients[1]
bS1 <- summary(modelS1)$coefficients[2]
clxS1 <- summary(modelS1)$coefficients[3]
round(clxS1 , digits = 0)
clxS1_SE <- summary(modelS1)$coefficients[6]
round(clxS1_SE , digits = 0)
clxS1_min <- clxS1 - clxS1_SE
clxS1_max <- clxS1 + clxS1_SE

plateauS1 <- aS1 + bS1 * clxS1 + (-0.5*bS1)*clxS1
plateauS1

SErangeS1min <- clxS1 - clxS1_SE
round(SErangeS1min , digits = 0)
SErangeS1max <- clxS1 + clxS1_SE
round(SErangeS1max , digits = 0)

Nrate_relyield_fit_quadplat <- fitted(modelS1)
Nrate_relyield_fit_quadplat_df <- data.frame(paper3_gsdata$N_level_kgha , Nrate_relyield_fit_quadplat) #creates dataframe
...
FigS1 <- ggplot(data = paper3_gsdata , aes ( x = N_level_kgha , y = relative_grain_yield)) +
  geom_point(data = paper3_gsdata , aes ( x = N_level_kgha , y = relative_grain_yield , shape = site_year) , size = 6) +
  geom_line(data = Nrate_relyield_fit_quadplat_df , aes( x = paper3_gsdata.N_level_kgha , y = Nrate_relyield_fit_quadplat) , size = 3 , color = "blue") +
  coord_cartesian(ylim = c(0 , 1) , xlim = c(0 , 252)) +
  scale_x_continuous(breaks = seq(0 , 250 , by = 50)) +
  scale_y_continuous(breaks = seq(0, 1 , by = .1)) +
  theme_classic() +
  labs(x = "Pre-plant N Rate ( kg N ha"^-1~")" , y = "Relative Grain Yield" , shape = "Site-Year") +
  geom_errorbar(mapping = aes(x = clxS1 , y = 0.5 , xmax = clxS1_max , xmin = clxS1_min) , width = 0.075 , size = .75) +
  ...
  geom_vline(xintercept = 1.832e+02 , linetype = "dashed" , size = 1) +
...
ggsave("FIGURES/FigS1.tiff" , FigS1, compression = "lzw" , width = 17 , height = 12 , type = "cairo" , dpi = 1000)
...
fit.lm1    <- lm(relative_grain_yield ~ PI_N_Uptake, data= paper3_gsdata)
...
model1 <- nls(relative_grain_yield ~ quadplat(PI_N_Uptake, a, b, clx),
            data = paper3_gsdata,
            start = list(a   = a.ini1,
                         b   = b.ini1,
                         clx = clx.ini1),
...
null1 <- nls(relative_grain_yield ~ nullfunct(PI_N_Uptake, m),
           data = paper3_gsdata,
           start = list(m   = m.ini1),
           trace = FALSE,
           nls.control(maxiter = 1000))
...
round(clx1 , digits = 0)
clx1_SE <- summary(model1)$coefficients[6]
round(clx1_SE , digits = 0)
clx1_max <- clx1 + clx1_SE
clx1_min <- clx1 - clx1_SE

plateau1 <- a1 + b1 * clx1 + (-0.5*b1)*clx1
plateau1
...
ggplot(data = paper3_gsdata , aes(x = PI_N_Uptake , y = relative_grain_yield)) +
  geom_point(data = paper3_gsdata , aes(x = PI_N_Uptake , y = relative_grain_yield))
...
Fig3 <- ggplot(data = paper3_gsdata , aes ( x = PI_N_Uptake , y = relative_grain_yield)) +
  geom_point(data = paper3_gsdata , aes ( x = PI_N_Uptake , y = relative_grain_yield , shape = site_year) , size = 6) +
  geom_line(data = pinup_yield_fit_linplat_df , aes( x = paper3_gsdata.PI_N_Uptake , y = pinup_yield_fit_linplat) , size = 3 , color = "blue") +
  coord_cartesian(ylim = c(0 , 1) , xlim = c(0 , 252)) +
  scale_x_continuous(breaks = seq(0 , 250 , by = 50)) +
  scale_y_continuous(breaks = seq(0, 1 , by = .1)) +
  theme_classic() +
  labs(x = "PI Total N Uptake ( kg N ha"^-1~")" , y = "Relative Grain Yield" , shape = "Site-Year") +
  geom_errorbar(mapping = aes(x = clx1 , y = 0.5 , xmax = clx1_max , xmin = clx1_min) , width = 0.075 , size = .75) +
  theme(axis.title = element_text(size = 32),
        axis.text = element_text(size = 32),
        legend.text = element_text(size = 20),
        legend.title = element_text(size = 20 , hjust = 0.5),
  ...
  geom_vline(xintercept = 109.31388375 , linetype = "dashed" , size = 1) +
  theme(legend.position = c(0.90 , 0.2)) +
  scale_shape_manual(values = c(1:20))
...
ggsave("FIGURES/Fig3.tiff" , Fig3, compression = "lzw" , width = 17 , height = 12 , type = "cairo" , dpi = 1000)
...
Fig4.1 <- ggplot(data = paper3_data , aes(x = Index) ) +
  geom_density(data = paper3_data, aes(x = Index  , color = Platform , linetype = Platform) , size = 1.5 , show.legend = F) +
  geom_rug(data = paper3_data , aes(x = Index  , color = Platform , linetype = Platform) , size = 1.2 , show.legend = T) +
  theme_classic() +
  labs(title = "(a)" , x = "Raw Vegetation Index" , y = "Density" , color = "Index" , linetype = "Index") +
  guides(color = guide_legend(keywidth = 7.5 , keyheight = 1.5 , unit = "cm" , title.hjust = 0.5 , byrow = TRUE),
         linetype = guide_legend(keywidth = 7.5 , keyheight = 1.5 , unit = "cm" , override.aes = list(size = 3) , title.hjust = 0.5 , byrow = TRUE)) +
  theme(axis.title = element_text(size = 32),
  ...
  coord_cartesian(xlim = c(0.225 , 0.95)  ,ylim = c(0 , 18)) +
  ...
  scale_linetype_manual(breaks = c("sUAS NDVI" , "GreenSeeker NDVI" ,  "sUAS NDRE") , values = c("dashed" , "solid" ,  "dotted") , labels = c(expression(NDVI[UAS]) , expression(NDVI[GS]) , expression(NDRE[UAS])))
  

Fig4.1

ggsave("FIGURES/NOT_IN_PUBLICATION/Fig4.1.tiff" , Fig4.1 , compression = "lzw" , width = 15 , height = 10, type = "cairo" , dpi = 500)
...
Fig4.2 <- ggplot(data = paper3_data , aes(x = SI) ) +
  geom_density(data = paper3_data, aes(x = SI  , color = Platform , linetype = Platform) , size = 1.5) +
  geom_rug(data = paper3_data , aes(x = SI  , color = Platform , linetype = Platform) , size = 1.2) +
  theme_classic() +
  labs(title = "(b)" , x = "Sufficiency-Index" , y = "Density" , color = "Index" , linetype = "Index") +
  theme(axis.title = element_text(size = 32),
        axis.text = element_text(size = 28),
        legend.text = element_text(size = 24),
  ...
  scale_x_continuous(breaks = seq(.2 , 1.2 , by = .1)) +
  scale_y_continuous(breaks = seq(0 , 18 , by = 3)) +
  scale_color_manual(breaks = c("sUAS NDVI" , "GreenSeeker NDVI" ,  "sUAS NDRE") , values = c( "dodgerblue" , "black" , "deeppink") , labels = c(expression(NDVI[UAS]) , expression(NDVI[GS]) , expression(NDRE[UAS]))) +
  scale_linetype_manual(breaks = c("sUAS NDVI" , "GreenSeeker NDVI" ,  "sUAS NDRE") , values = c("dashed" , "solid" ,  "dotted") , labels = c(expression(NDVI[UAS]) , expression(NDVI[GS]) , expression(NDRE[UAS]))) +
    guides(
...
ggsave("FIGURES/NOT_IN_PUBLICATION/Fig4.2.tiff" , Fig4.2 , compression = "lzw" , width = 15 , height = 10, type = "cairo" , dpi = 500)
...
Fig4 <- grid.arrange(arrangeGrob(Fig4.1,
                                  Fig4.2.2,
                                  nrow = 1,
                                  ncol = 2
                                  ))

ggsave("FIGURES/Fig4.tiff" , Fig4 , compression = "lzw" , width = 20 , height = 10, type = "cairo" , dpi = 1000)
...
a.ini <- summary(model)$coefficients[1]
b.ini <- summary(model)$coefficients[2]
c.ini <- summary(model)$coefficients[3]
...
round(nagelkerke(model,
           null)$Pseudo.R.squared.for.model.vs.null[2] , digits = 2)
...
GS_ndvi_si_quad_plot <- ggplot(data = paper3_gsdata , aes ( x = PI_N_Uptake , y = SI)) +
  geom_point(data = paper3_gsdata , aes ( x = PI_N_Uptake , y = SI , shape = site_year) , size = 7) +
  geom_line(data = GS_ndvi_quadplat_df , aes( x = paper3_gsdata.PI_N_Uptake , y = GS_ndvi_fit_quadplat) , size = 3 , color = "blue") +
  coord_cartesian(ylim = c(0.20 , 1.2) , xlim = c(0 , 252)) +
  scale_x_continuous(breaks = seq(0 , 250 , by = 50)) +
  scale_y_continuous(breaks = seq(.2 , 1.2 , by = .2)) +
  theme_classic() +
  labs(title = "(a)" , x = NULL , y = NULL) +
  annotate("text", x = 175 , y = .34, label = "NDVI[GS]", size = 10 , color = "black" , hjust = .5 , parse = T) +
  annotate("text", x = 175 , y = .27, label = "plateau~'='~111~kg~N~ha^-1", size = 9 , color = "black" , parse = T , hjust = .5) +
  annotate("text", x = 175 , y = .2, label = "R² = 0.75", size = 9 , color = "black"  , hjust = .5) +
...
round(nagelkerke(model2,
           null2)$Pseudo.R.squared.for.model.vs.null[2] , digits = 2)
...
suas_ndre_si_quad_plot <- ggplot(data = paper3_uas_ndre_data , aes ( x = PI_N_Uptake , y = SI)) +
  geom_point(data = paper3_uas_ndre_data , aes ( x = PI_N_Uptake , y = SI , shape = site_year) , size = 7) +
  ...
  annotate("text", x = 175 , y = .34, label = "NDRE[UAS]", size = 10 , color = "black" , hjust = .5 , parse = T) +
  annotate("text", x = 175 , y = .27, label = "plateau~'='~130~kg~N~ha^-1", size = 9 , color = "black" , parse = T , hjust = .5) +
  annotate("text", x = 175 , y = .2, label = "R² = 0.79", size = 9 , color = "black"  , hjust = .5) +
  theme(axis.title = element_text(size = 30),
        axis.text = element_text(size = 30),
        legend.text = element_text(size = 24),
        legend.title = element_text(size = 24),
...
round(nagelkerke(model3,
           null3)$Pseudo.R.squared.for.model.vs.null[2] , digits = 2)
...
suas_ndvi_si_quad_plot <- ggplot(data = paper3_uas_ndvi_data , aes ( x = PI_N_Uptake , y = SI)) +
  geom_point(data = paper3_uas_ndvi_data , aes ( x = PI_N_Uptake , y = SI , shape = site_year) , size = 7) +
  geom_line(data = sUAS_ndvi_quadplat_df , aes( x = paper3_uas_ndvi_data.PI_N_Uptake , y = sUAS_ndvi_fit_quadplat) , size = 3 , color = "blue") +
  coord_cartesian(ylim = c(0.20 , 1.2) , xlim = c(0 , 252)) +
  scale_x_continuous(breaks = seq(0 , 250 , by = 50)) +
  scale_y_continuous(breaks = seq(.2 , 1.2 , by = .2)) +
  theme_classic() +
  labs(title = "(c)" , x = NULL , y = NULL , shape = "Site-Year") +
  annotate("text", x = 175 , y = .34, label = "NDVI[UAS]", size = 10 , color = "black" , hjust = .5 , parse = T) +
  annotate("text", x = 175 , y = .27, label = "plateau~'='~96~kg~N~ha^-1", size = 9 , color = "black" , parse = T , hjust = .5) +
  annotate("text", x = 175 , y = .2, label = "R² = 0.82", size = 9 , color = "black"  , hjust = .5) +
  theme(axis.title = element_text(size = 30),
        axis.text = element_text(size = 30),
        ...
        plot.title = element_text(size = 38 , hjust = 0.5)) +
  geom_hline(yintercept = 0 , size = 0.75) +
  theme(legend.position = c(.66 , .425)) +
  scale_shape_manual(values = c(1:20)) +
...
Fig5 <- grid.arrange(arrangeGrob(GS_ndvi_si_quad_plot,
                                 suas_ndre_si_quad_plot,
                                 suas_ndvi_si_quad_plot,
                                 bottom = textGrob("PI N Uptake ( kg N ha"^-1~")" , gp = gpar(fontsize = 40)),
                                 nrow = 1,
                                 ncol = 3
                                  ))

title <- textGrob("Sufficiency-Index" , rot = 90 , gp = gpar(fontsize = 40))

title <- arrangeGrob(zeroGrob() , title, zeroGrob(), 
                    heights = unit(1, 'npc'), 
                    widths = unit(c(.2 , 1, .2), c('cm' , 'cm', 'cm')),
                    as.table = FALSE)

Fig5 <- grid.arrange(arrangeGrob(Fig5, left = title))

ggsave("FIGURES/Fig5.tiff" , Fig5 , compression = "lzw" , width = 22 , height = 10, type = "cairo" , dpi = 1000)
...
fit.lmS3_1    <- lm(SI ~ N_level_kgha , data = paper3_gsdata)

a.iniS3_1     <- fit.lmS3_1$coefficients[1]
b.iniS3_1     <- fit.lmS3_1$coefficients[2]
clx.iniS3_1   <- mean(paper3_gsdata$N_level_kgha)

quadplat <- function(x, a, b, clx) {
           ifelse(x  < clx, a + b * x   + (-0.5*b/clx) * x   * x,
                            a + b * clx + (-0.5*b/clx) * clx * clx)}

modelS3_1 <- nls(SI ~ quadplat(N_level_kgha, a, b, clx),
            data = paper3_gsdata,
            start = list(a   = a.iniS3_1,
                         b   = b.iniS3_1,
                         clx = clx.iniS3_1),
             trace = FALSE,
             nls.control(maxiter = 1000))
...
m.iniS3_1    <- mean(paper3_gsdata$N_level_kgha)

nullS3_1 <- nls(SI ~ nullfunct(N_level_kgha, m),
           data = paper3_gsdata,
           start = list(m   = m.iniS3_1),
           trace = FALSE,
           nls.control(maxiter = 1000))

nagelkerke(modelS3_1,
           nullS3_1)

confint2(modelS3_1,
         level = 0.95)

BootS3_1 <- nlsBoot(modelS3_1)
...
plotPredy(data  = paper3_gsdata,
          ...
          model = modelS3_1,
          xlab  = "Pre-plant N Rate",
          ylab  = "Sufficiency-Index")

xS3_1 <- residuals(modelS3_1)

plotNormalHistogram(xS3_1)

plot(fitted(modelS3_1),
     residuals(modelS3_1))

aS3_1 <- summary(modelS3_1)$coefficients[1]
bS3_1 <- summary(modelS3_1)$coefficients[2]
clxS3_1 <- summary(modelS3_1)$coefficients[3]
round(clxS3_1)
clxS3_1_SE <- summary(modelS3_1)$coefficients[6]
round(clxS3_1_SE)

plateauS3_1 <- aS3_1 + bS3_1 * clxS3_1 + (-0.5*bS3_1)*clxS3_1
plateauS3_1

Nrate_SI_gsndvi_fit_quadplat <- fitted(modelS3_1)
Nrate_SI_gsndvi_fit_quadplat_df <- data.frame(paper3_gsdata$N_level_kgha , Nrate_SI_gsndvi_fit_quadplat) #creates dataframe

Nrate_SI_gsndvi_fit_quadplat_df <- Nrate_SI_gsndvi_fit_quadplat_df %>% 
  rename(N_rate = paper3_gsdata.N_level_kgha,
         Fit = Nrate_SI_gsndvi_fit_quadplat) %>% 
...
fit.lmS3_2    <- lm(SI ~ N_level_kgha , data = paper3_uas_ndvi_data)

a.iniS3_2     <- fit.lmS3_2$coefficients[1]
b.iniS3_2     <- fit.lmS3_2$coefficients[2]
clx.iniS3_2   <- mean(paper3_uas_ndvi_data$N_level_kgha)

quadplat <- function(x, a, b, clx) {
           ifelse(x  < clx, a + b * x   + (-0.5*b/clx) * x   * x,
                            a + b * clx + (-0.5*b/clx) * clx * clx)}

modelS3_2 <- nls(SI ~ quadplat(N_level_kgha, a, b, clx),
            data = paper3_uas_ndvi_data,
            start = list(a   = a.iniS3_2,
                         b   = b.iniS3_2,
                         clx = clx.iniS3_2),
             trace = FALSE,
             nls.control(maxiter = 1000))
...
m.iniS3_2    <- mean(paper3_uas_ndvi_data$N_level_kgha)

nullS3_2 <- nls(SI ~ nullfunct(N_level_kgha, m),
           data = paper3_uas_ndvi_data,
           start = list(m   = m.iniS3_2),
           trace = FALSE,
           nls.control(maxiter = 1000))

nagelkerke(modelS3_2,
           nullS3_2)

confint2(modelS3_2,
         level = 0.95)

BootS3_2 <- nlsBoot(modelS3_2)
...
xS3_2 <- residuals(modelS3_2)

plotNormalHistogram(xS3_2)

plot(fitted(modelS3_2),
     residuals(modelS3_2))

aS3_2 <- summary(modelS3_2)$coefficients[1]
bS3_2 <- summary(modelS3_2)$coefficients[2]
clxS3_2 <- summary(modelS3_2)$coefficients[3]
round(clxS3_2)
clxS3_2_SE <- summary(modelS3_2)$coefficients[6]
round(clxS3_2_SE)

plateauS3_2 <- aS3_2 + bS3_2 * clxS3_2 + (-0.5*bS3_2)*clxS3_2
plateauS3_2

Nrate_SI_uasNDVI_fit_quadplat <- fitted(modelS3_2)
Nrate_SI_uasNDVI_fit_quadplat_df <- data.frame(paper3_uas_ndvi_data$N_level_kgha , Nrate_SI_uasNDVI_fit_quadplat) #creates dataframe

Nrate_SI_uasNDVI_fit_quadplat_df <- Nrate_SI_uasNDVI_fit_quadplat_df %>% 
  rename(N_rate = paper3_uas_ndvi_data.N_level_kgha,
         Fit = Nrate_SI_uasNDVI_fit_quadplat) %>% 
  mutate(Index = "uas_NDVI",
         SI = paper3_uas_ndvi_data$SI)
...
fit.lmS3_3    <- lm(SI ~ N_level_kgha , data = paper3_uas_ndre_data)

a.iniS3_3     <- fit.lmS3_3$coefficients[1]
b.iniS3_3     <- fit.lmS3_3$coefficients[2]
clx.iniS3_3   <- mean(paper3_uas_ndre_data$N_level_kgha)

quadplat <- function(x, a, b, clx) {
           ifelse(x  < clx, a + b * x   + (-0.5*b/clx) * x   * x,
                            a + b * clx + (-0.5*b/clx) * clx * clx)}

modelS3_3 <- nls(SI ~ quadplat(N_level_kgha, a, b, clx),
            data = paper3_uas_ndre_data,
            start = list(a   = a.iniS3_3,
                         b   = b.iniS3_3,
                         clx = clx.iniS3_3),
             trace = FALSE,
             nls.control(maxiter = 1000))
...
m.iniS3_3    <- mean(paper3_uas_ndre_data$N_level_kgha)

nullS3_3 <- nls(SI ~ nullfunct(N_level_kgha, m),
           data = paper3_uas_ndre_data,
           start = list(m   = m.iniS3_3),
           trace = FALSE,
           nls.control(maxiter = 1000))

nagelkerke(modelS3_3,
           nullS3_3)

confint2(modelS3_3,
         level = 0.95)

BootS3_3 <- nlsBoot(modelS3_3)
...
xS3_3 <- residuals(modelS3_3)

plotNormalHistogram(xS3_3)

plot(fitted(modelS3_3),
     residuals(modelS3_3))

aS3_3 <- summary(modelS3_3)$coefficients[1]
bS3_3 <- summary(modelS3_3)$coefficients[2]
clxS3_3 <- summary(modelS3_3)$coefficients[3]
round(clxS3_3)
clxS3_3_SE <- summary(modelS3_3)$coefficients[6]
round(clxS3_3_SE)

plateauS3_3 <- aS3_3 + bS3_3 * clxS3_3 + (-0.5*bS3_3)*clxS3_3
plateauS3_3

Nrate_SI_uasndre_fit_quadplat <- fitted(modelS3_3)
Nrate_SI_uasndre_fit_quadplat_df <- data.frame(paper3_uas_ndre_data$N_level_kgha , Nrate_SI_uasndre_fit_quadplat) #creates dataframe

Nrate_SI_uasndre_fit_quadplat_df <- Nrate_SI_uasndre_fit_quadplat_df %>% 
  rename(N_rate = paper3_uas_ndre_data.N_level_kgha,
         Fit = Nrate_SI_uasndre_fit_quadplat) %>% 
  mutate(Index = "uas_NDRE",
         SI = paper3_uas_ndre_data$SI)

figS3_data <- rbind(Nrate_SI_gsndvi_fit_quadplat_df,
                    Nrate_SI_uasNDVI_fit_quadplat_df,
                    Nrate_SI_uasndre_fit_quadplat_df)

figS3_data$Index <- as.factor(figS3_data$Index)
```
...
Fig6 <- ggplot(data = figS3_data , aes ( x = N_rate , y = SI)) +
  geom_point(data = figS3_data , aes ( x = N_rate , y = SI , shape = Index , color = Index) , size = 6 ) +
  geom_line(data = figS3_data , aes( x = N_rate , y = Fit , color = Index , linetype = Index) , size = 3) +
  coord_cartesian(ylim = c(0 , 1) , xlim = c(0 , 252)) +
  scale_x_continuous(breaks = seq(0 , 250 , by = 50)) +
  scale_y_continuous(breaks = seq(0, 1 , by = .1)) +
  theme_classic() +
  labs(x = "Pre-plant N Rate ( kg N ha"^-1~")" , y = "Sufficiency-Index" , shape = "Index" , color = "Index" , linetype = "Index") +
  guides(color = guide_legend(keywidth = 10 , keyheight = 1.5 , unit = "cm" , title.hjust = 0.5 , byrow = TRUE),
         shape = guide_legend(keywidth = 10 , keyheight = 1.5 , unit = "cm" , title.hjust = 0.5 , byrow = TRUE),
         linetype = guide_legend(keywidth = 10 , keyheight = 1.5 , unit = "cm" , title.hjust = 0.5 , byrow = TRUE)) +
  theme(axis.title = element_text(size = 32),
        axis.text = element_text(size = 32),
  ...
  geom_vline(xintercept = clxS3_1 , linetype = "solid" , size = 1.5 , color = "black") +
  geom_vline(xintercept = clxS3_2 , linetype = "longdash" , size = 1.5 , color = "dodgerblue") +
  geom_vline(xintercept = clxS3_3 , linetype = "dashed" , size = 1.5 , color = "deeppink") +
  scale_shape_manual("Index" , breaks = c("uas_NDVI" , "gs_NDVI" ,  "uas_NDRE") , values = c( 1 , 2 , 0) , labels = c(expression(NDVI[UAS]) , expression(NDVI[GS]) , expression(NDRE[UAS]))) +
  scale_color_manual("Index" , breaks = c("uas_NDVI" , "gs_NDVI" ,  "uas_NDRE") , values = c( "dodgerblue" , "black" , "deeppink") , labels = c(expression(NDVI[UAS]) , expression(NDVI[GS]) , expression(NDRE[UAS]))) +
  scale_linetype_manual("Index" , breaks = c("uas_NDVI" , "gs_NDVI" ,  "uas_NDRE") , values = c("longdash" , "solid" ,  "dashed") , labels = c(expression(NDVI[UAS]) , expression(NDVI[GS]) , expression(NDRE[UAS])))

Fig6

ggsave("FIGURES/Fig6.tiff" , Fig6, compression = "lzw" , width = 17 , height = 12 , type = "cairo" , dpi = 1000)
...
round(r.squaredGLMM(gsndvi_yield_mixlm)[1] , digits = 2)

gsndvi_yield_slope <- round((summary(gsndvi_yield_mixlm)$tTable[2] * 0.1)  , digits = 1)
...
mylist <- list(SI=seq(round(min(paper3_gsdata$SI), digits = 3) , round(max(paper3_gsdata$SI), digits = 3) , by = .001))
...
plot_si1 <- ggplot( data = paper3_gsdata , aes( x = SI , y = GrainYield_Mgha)) +
  geom_point( data = paper3_gsdata , aes( x = SI , y = GrainYield_Mgha , shape = site_year) , size = 8) +
  ...
  coord_cartesian(ylim = c(0,14) , xlim = c(.2 , 1.15)) +
  theme(axis.text = element_text(size = 38),
        axis.title = element_text(size = 38),
        legend.text = element_text(size = 24),
        legend.title = element_text(size = 24),
        legend.position = "none") +
  annotate("text", x=0.1625, y=13.8, label="NDVI[GS]", size=11 , color = "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=12.5, label="R^2~'='~0.54", size=10 , color= "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=11, label="italic(p) < 0.001", size=10 , color= "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=9.8, label="slope~'='~0.9~Mg~ha^-1~per~0.1~SI", size=10 , color= "black" , hjust = 0 , parse = T) +
  scale_shape_manual(values = c(1:20))
...
round(r.squaredGLMM(sUASndre_yield_mixlm)[1] , digits = 2)

uas_ndre_yield_slope <- round((summary(sUASndre_yield_mixlm)$tTable[2] * 0.1)  , digits = 1)

uas_ndre_yield_slope
...
mylist <- list(SI=seq(round(min(paper3_uas_ndre_data$SI), digits = 3) , round(max(paper3_uas_ndre_data$SI), digits = 3) , by = .001))
...
plot_si2 <- ggplot( data = paper3_uas_ndre_data , aes( x = SI , y = GrainYield_Mgha)) +
  geom_point( data = paper3_uas_ndre_data , aes( x = SI , y = GrainYield_Mgha , shape = site_year) , size = 8) +
  geom_line( data = sUASndre_emmeans , aes(x = SI , y = emmean) , color = "blue" , size = 3.5) +
  theme_classic() +
  labs( x = NULL , y = NULL , shape = "Site-Year") +
  scale_x_continuous(breaks = seq(0.2 , 1.2 , by = .1)) +
  scale_y_continuous(breaks = seq(0 , 14 , by = 4)) +
  coord_cartesian(ylim = c(0,14) , xlim = c(0.2 , 1.15)) +
  theme(axis.text = element_text(size = 38),
        axis.title = element_text(size = 38),
        legend.text = element_text(size = 20),
        legend.title = element_text(size = 20),
        legend.position = "none") +
  annotate("text", x=0.1625, y=13.8, label="NDRE[UAS]", size=11 , color = "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=12.5, label="R^2~'='~0.51", size=10 , color= "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=11, label="italic(p) < 0.001", size=10 , color= "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=9.8, label="slope~'='~1.1~Mg~ha^-1~per~0.1~SI", size=10 , color= "black" , hjust = 0 , parse = T) +
  scale_shape_manual(values = c(1:20))
...
round(r.squaredGLMM(sUASndvi_yield_mixlm)[1] , digits = 2)

sUASndvi_yield_slope <- round((summary(sUASndvi_yield_mixlm)$tTable[2] * 0.1)  , digits = 1)

sUASndvi_yield_slope
```
...
mylist <- list(SI=seq(round(min(paper3_uas_ndvi_data$SI), digits = 3) , round(max(paper3_uas_ndvi_data$SI) , digits = 3) , by = .001))
...
plot_si3 <- ggplot( data = paper3_uas_ndvi_data , aes( x = SI , y = GrainYield_Mgha)) +
  geom_point( data = paper3_uas_ndvi_data , aes( x = SI , y = GrainYield_Mgha , shape = site_year) , size = 8) +
  ...
  theme(axis.text = element_text(size = 38),
        ...
        legend.text = element_text(size = 20),
        legend.title = element_text(size = 20 , hjust = 0.5),
        legend.position = c(.065 , .315),
        legend.box.background = element_rect(size = 1)) +
  annotate("text", x=0.1625, y=13.8, label="NDVI[UAS]", size=11 , color = "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=12.5, label="R^2~'='~0.55", size=10 , color= "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=11, label="italic(p) < 0.001", size=10 , color= "black" , hjust = 0 , parse = T) +
  annotate("text", x=0.1625, y=9.8, label="slope~'='~2.4~Mg~ha^-1~per~0.1~SI", size=10 , color= "black" , hjust = 0 , parse = T) +
  scale_shape_manual(values = c(1:20))  +
...
a <- textGrob("(a)" , hjust = 0.5 , gp = gpar(fontsize = 40))


b <- textGrob("(b)" , hjust = 0.5 , gp = gpar(fontsize = 40))


c <- textGrob("(c)" , hjust = 0.5 , gp = gpar(fontsize = 40))


axis_title <- textGrob("Sufficiency-Index" , hjust = 0.5 , gp = gpar(fontsize = 44))


Fig7 <- grid.arrange(arrangeGrob(zeroGrob(),
                                 a, 
                                 zeroGrob(),
                                 plot_si1 , 
                                 zeroGrob(),
...
ggsave("FIGURES/Fig7.tiff" , Fig7 , compression = "lzw" , width = 20 , height = 22, type = "cairo" , dpi = 1000)
```
